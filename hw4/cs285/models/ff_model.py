...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        obs_normalized = normalize(ptu.from_numpy(obs_unnormalized), ptu.from_numpy(obs_mean), ptu.from_numpy(obs_std)) # TODO(Q1)
        acs_normalized = normalize(ptu.from_numpy(acs_unnormalized), ptu.from_numpy(acs_mean), ptu.from_numpy(acs_std)) # TODO(Q1)
        ...
        next_obs_pred = ptu.from_numpy(obs_unnormalized) + unnormalize(delta_pred_normalized, ptu.from_numpy(delta_mean), ptu.from_numpy(delta_std)) # TODO(Q1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        loss = self.loss(self(observations, actions, **data_statistics)[1], ptu.from_numpy(target))# TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
