...
class DynamicsModel(nn.Module, BaseExplorationModel):
    def __init__(self, hparams, optimizer_spec, **kwargs):
        super().__init__(**kwargs)
        self.ob_dim = hparams['ob_dim']
        self.ac_dim = 1 # hard-coded here for some reason it does not work
        self.output_size = self.ob_dim
        self.n_layers = hparams['rnd_n_layers']
        self.size = hparams['rnd_size']
        self.optimizer_spec = optimizer_spec
        ...
        self.f = ptu.build_mlp(input_size=self.ob_dim + self.ac_dim, output_size=self.output_size, n_layers=self.n_layers,
                               size=self.size, init_method=init_method_1)
    ...
    def forward(self, ob_no, ac_na, next_ob_no):
        # TODO: Get the prediction error for ob_no
        # HINT: Remember to detach the output of self.f!
        ac_na = torch.unsqueeze(ac_na, 1)
        concatenated_input = torch.cat([ob_no, ac_na], dim=1)
        error = next_ob_no - self.f(concatenated_input)
        return torch.norm(error, dim=1) ** 2

    def forward_np(self, ob_no, ac_na, next_ob_no):
        ob_no = ptu.from_numpy(ob_no)
    ...
    def update(self, ob_no, ac_na, next_ob_no):
        # TODO: Update f
        ob_no = ptu.from_numpy(ob_no)
        ac_na = ptu.from_numpy(ac_na)
        next_ob_no = ptu.from_numpy(next_ob_no)
        loss = torch.mean(self(ob_no, ac_na, next_ob_no))
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        self.learning_rate_scheduler.step()
